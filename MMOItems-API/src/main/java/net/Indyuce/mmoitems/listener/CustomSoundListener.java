...
public class CustomSoundListener implements Listener {
    ...
    public void d(PlayerClickEvent event) {
        if (!event.hasItem()) return;

        var customSound = event.isLeftClick() ? CustomSound.ON_LEFT_CLICK : CustomSound.ON_RIGHT_CLICK;
        playSound(event.getItem(), customSound, event.getPlayer());
    }
