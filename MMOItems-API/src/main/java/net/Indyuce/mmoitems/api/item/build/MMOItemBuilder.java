...
public class MMOItemBuilder extends Buildable<MMOItem> {
    ...
    public MMOItemBuilder(@NotNull MMOItemTemplate template, int level, @Nullable ItemTier tier, boolean forDisplay) {
        ...
        // Defaults to tier tooltip if not provided by template
        ...
        // Defaults to tier/type tooltip style if not provided by template
        if (tier != null && tier.getTooltipStyle() != null && !mmoitem.hasData(ItemStats.TOOLTIP_STYLE))
            mmoitem.setData(ItemStats.TOOLTIP_STYLE, new StringData(tier.getTooltipStyle().toString()));
        else if (template.getType().getVanillaTooltip() != null && !mmoitem.hasData(ItemStats.TOOLTIP_STYLE))
            mmoitem.setData(ItemStats.TOOLTIP_STYLE, new StringData(template.getType().getVanillaTooltip().toString()));
