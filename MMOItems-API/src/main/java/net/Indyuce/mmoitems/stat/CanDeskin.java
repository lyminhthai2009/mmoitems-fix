...
public class CanDeskin extends BooleanStat implements ConsumableItemInteraction {
    ...
    public boolean handleConsumableEffect(@NotNull InventoryClickEvent event, @NotNull PlayerData playerData, @NotNull Consumable consumable, @NotNull NBTItem target, Type targetType) {
        ...
        if (consumable.getNBTItem().getBoolean("MMOITEMS_CAN_DESKIN") && !skinId.isEmpty()) {
            ...
            targetItem.setType(originalItem.getType()); // important, fixes item meta
            ...
            if (MythicLib.plugin.getVersion().isAbove(1, 21, 2)) {
                if (originalItemMeta.hasEquippable()) {
                    ...
                    //   Bukkit.broadcastMessage("og has no equipable");
                    // hacky fix for component problem
                    // items cannot be re-equipped before player relogin
                    // https://gitlab.com/phoenix-dvpmt/mmoitems/-/issues/1863
                    //   var vanillaComponent = new ItemStack(originalItem.getType()).getItemMeta().getEquippable();
                    //   Bukkit.broadcastMessage("equippable = " + vanillaComponent);
