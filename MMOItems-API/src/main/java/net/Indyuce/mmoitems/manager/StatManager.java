...
public class StatManager {
    ...
    @BackwardsCompatibility(version = "6.10-SNAPSHOT")
    public void fixLegacyCustomStatIds() {
        final var configFile = new ConfigFile(MMOItems.plugin, "custom-stats");
        final var config = configFile.getConfig();
        final var legacySection = config.getConfigurationSection("custom-stats");
        for (var key : legacySection.getKeys(false)) {
            var obj = legacySection.get(key);
            if (obj instanceof ConfigurationSection) {
                final String name = ((ConfigurationSection) obj).getString("name", key);
                final String legacyStatId = String.format("custom_%s", name.replace(" ", "_")).toUpperCase();
                config.set(legacyStatId, obj);
            }
            // Syntax error, skip, user has to deal with it manually
            else {
                config.set("CUSTOM_" + key.toUpperCase(), obj);
            }
        }
        config.set("custom-stats", null);
        configFile.save();
    }
    ...
    private void loadCustomStats() {
        ...
        final var config = new ConfigFile("custom-stats").getConfig();
        for (var key : config.getKeys(false))
            try {
                var configSection = config.getConfigurationSection(key);
                Validate.notNull(configSection, "Custom stat section " + key + " is null");
                registerCustomStat(configSection);
            } catch (Exception exception) {
                MMOItems.plugin.getLogger().log(Level.WARNING, "Could not load custom stat '" + key + "': " + exception.getMessage());
            }
    }
    ...
    private void registerCustomStat(@NotNull ConfigurationSection section) {
        ...
        final String statId = section.getName();
