...
public class ItemStackBuilder {
    ...
    public NBTItem buildNBT(boolean forDisplay) {
        ...
        // Call event, only synchronously
        final List<String> effectiveLore;
        if (Bukkit.isPrimaryThread()) {
            var called = new GenerateLoreEvent(builtMMOItem, lore, parsedLore, unparsedLore);
            Bukkit.getPluginManager().callEvent(called);
            effectiveLore = called.getParsedLore();
        } else effectiveLore = parsedLore;

        // Apply lore to item
        AdventureUtils.setLore(meta, effectiveLore);
        ...
        effectiveLore.forEach(array::add);
